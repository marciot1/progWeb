    ...
    print(image)
      try:
        ...
        if request.FILES is not None:
          ...
          if num_files > 0: imagefile = request.FILES['image']
            print(imagefile)
            fs = FileSystemStorage()
            filename = fs.save(imagefile.name, imagefile)
            if (filename is not None) and (filename != ""): obj_produto.image = filename
          obj_produto.save()
...
# adicione a função que trata o postback da interface de 
